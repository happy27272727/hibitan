...
// 特定ユーザー専用カスタマイズ（3目標チェックリスト）
const SPECIAL_USER_IDS = [98, 165, 38];
function isSpecialUser(u) {
    return !!u && SPECIAL_USER_IDS.includes(Number(u.登録番号));
}
let goalCheckState = [false, false, false]; // ローカルのみ、当日限り。「記録」時にまとめてDB反映
let goalDayCounts = [0, 0, 0];
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (res && !res.error && res.data && res.data[0]) {
        ...
        onUserReady(user);
        await updateHome();
        if (!isSpecialUser(user)) renderCalendar(currentYear, currentMonth);
...
function bindButtons() {
    ...
    document.querySelectorAll('.goal-checkbox').forEach(cb => {
        cb.addEventListener('change', () => {
            goalCheckState[Number(cb.dataset.index)] = cb.checked;
            if (isSpecialUser(user)) upsertGoalsRemote();
        });
    });
    ...
    if (saveTemplateBtn) saveTemplateBtn.addEventListener('click', saveScheduleTemplate);

    const loadTemplateBtn = document.getElementById('loadTemplateBtn');
    if (loadTemplateBtn) loadTemplateBtn.addEventListener('click', loadScheduleTemplate);
}
...
async function updateHome() {
    ...
    document.getElementById('renzokuHyouzi').textContent = '実施日数： ' + (user.連続日数 || 0) + '日';

    if (isSpecialUser(user)) {
        relocateCalendarToOwnScreen();
        document.getElementById('mokuhyouHyouzi').style.display = 'none';
        document.getElementById('renzokuHyouzi').style.display = 'none';
        document.getElementById('goalChecklistContainer').style.display = 'block';
        document.getElementById('calendarNavBtn').style.display = 'inline-block';
        await initGoalCheckState();
        renderGoalChecklist();
        await loadGoalDayCounts();
        renderGoalCounts();
    } else {
        relocateCalendarToHome();
        document.getElementById('mokuhyouHyouzi').style.display = '';
        document.getElementById('renzokuHyouzi').style.display = '';
        document.getElementById('goalChecklistContainer').style.display = 'none';
        document.getElementById('calendarNavBtn').style.display = 'none';
    }
}
...
async function renderCalendar(year, month) {
    ...
    for (let d = 1; d <= lastDate; d++) {
        ...
        if (recordMap[dateStr]) {
            const mark = getCalendarMark(recordMap[dateStr]);
            const markHtml = mark === '◎' ? `<span class="calendar-mark-perfect">◎</span>` : mark;
            cell.innerHTML = `${d}\n${markHtml}`;
...
function configuredGoalNames() {
    return [user.目標, user.goal2_name, user.goal3_name];
}

// 設定済みの目標を全部達成なら◎、一部だけなら⭕、0個なら❌
function getCalendarMark(record) {
    if (record.goals_data && record.goals_data.length === 3) {
        const names = configuredGoalNames();
        let configuredCount = 0;
        let doneCount = 0;
        for (let i = 0; i < 3; i++) {
            if (!names[i]) continue;
            configuredCount++;
            if (record.goals_data[i] === true) doneCount++;
        }
        if (configuredCount > 0 && doneCount === configuredCount) return '◎';
        if (doneCount > 0) return '⭕';
        return '❌';
    }
    return record.achieved === true ? '⭕' : '❌';
}

function showDailyDetail(record) {
    let message = `${record.record_date}\n\n${record.diary || '（日記なし）'}`;

    if (record.goals_data && record.goals_data.length === 3) {
        const names = configuredGoalNames();
        const goalsText = names
            .map((name, i) => name ? `${name}：${record.goals_data[i] === true ? '⭕' : '❌'}` : null)
            .filter(Boolean)
            .join('\n');
        message = `${record.record_date}\n\n${goalsText}\n\n${record.diary || '（日記なし）'}`;
    }
...
// =============================================
// 10.5 目標チェックリスト（登録番号98専用）
// =============================================
function relocateCalendarToOwnScreen() {
    const calendarContainer = document.getElementById('calendarContainer');
    const calendarScreenBody = document.getElementById('calendarScreenBody');
    if (calendarContainer && calendarScreenBody && calendarContainer.parentElement !== calendarScreenBody) {
        calendarScreenBody.appendChild(calendarContainer);
    }
}

function relocateCalendarToHome() {
    const calendarContainer = document.getElementById('calendarContainer');
    const userGoal = document.getElementById('userGoal');
    if (calendarContainer && userGoal && calendarContainer.nextElementSibling !== userGoal) {
        userGoal.parentElement.insertBefore(calendarContainer, userGoal);
    }
}

async function initGoalCheckState() {
    ...
    const { data } = await supabase
        ...
        .maybeSingle();
    goalCheckState = data?.goals_data?.length === 3 ? data.goals_data : [false, false, false];
}

// チェックのON/OFFのたびに即DBへ反映する（「記録」を押し忘れても達成が失われないように）
async function upsertGoalsRemote() {
    const today = new Date().toISOString().split('T')[0];

    const { error } = await supabase.rpc('upsert_goals', {
        p_id:    user.登録番号,
        p_pass:  user.パスワード,
        p_date:  today,
        p_goals: goalCheckState
    });
    ...
    const { data } = await supabase.rpc('get_user_by_id', { p_id: user.登録番号 });
    if (data && data[0]) {
        user.連続日数 = data[0].連続日数;
        user.実施状況 = data[0].実施状況;
    }
...
function renderGoalChecklist() {
    const names = [user.目標, user.goal2_name, user.goal3_name];
    for (let i = 0; i < 3; i++) {
        const row = document.getElementById(`goalRow${i}`);
        if (i > 0 && !names[i]) {
            row.style.display = 'none';
            continue;
        }
        row.style.display = 'flex';
        document.getElementById(`goalName${i}`).textContent = names[i] || `目標${i + 1}`;
        const checkbox = document.querySelector(`.goal-checkbox[data-index="${i}"]`);
        if (checkbox) checkbox.checked = !!goalCheckState[i];
    }
}

function tallyGoalDayCounts(rows) {
    const counts = [0, 0, 0];
    if (!rows) return counts;

    rows.forEach(row => {
        if (row.goals_data && row.goals_data.length === 3) {
            for (let i = 0; i < 3; i++) {
                if (row.goals_data[i] === true) counts[i]++;
            }
        } else if (row.achieved === true) {
            // goals_data導入前の記録は目標1（元々の「目標」）の達成として引き継ぐ
            counts[0]++;
        }
    });
    return counts;
...
async function loadGoalDayCounts() {
    const { data, error } = await supabase
        .from('daily_records')
        .select('achieved, goals_data')
        .eq('user_id', user.登録番号);

    goalDayCounts = error ? [0, 0, 0] : tallyGoalDayCounts(data);
}

function renderGoalCounts() {
    for (let i = 0; i < 3; i++) {
        const el = document.getElementById(`goalCount${i}`);
        if (el) el.textContent = `${goalDayCounts[i]}日達成`;
    }
}
...
async function recordToday() {
    ...
    if (user.最終実施日 === today) {
        if (selectedPhotoFile) {
            ...
            if (existing?.photo_url) {
                alert('今日の写真はすでに投稿済みです');
                return;
            }
            const recordBtn = document.getElementById('tasseiButton');
            recordBtn.textContent = '写真アップロード中...';
            recordBtn.disabled = true;
            const photoUrl = await uploadPhoto(selectedPhotoFile, user.登録番号, today);
            recordBtn.textContent = '記録';
            recordBtn.disabled = false;
            if (photoUrl) {
                const { error } = await supabase.rpc('update_daily_photo', {
                    p_id: user.登録番号,
                    p_date: today,
    ...
    const { error } = await supabase.rpc('record_today', {
        ...
        p_date:      today,
        p_photo_url: photoUrl,
        p_goals:     isSpecialUser(user) ? goalCheckState : null
    });
    ...
    const achieved = isSpecialUser(user) ? goalCheckState.every(Boolean) : true;

    if (isSpecialUser(user)) {
        // 連続日数はサーバー側で達成状態の変化分だけ増減されるため、最新値を取り直す
        const { data } = await supabase.rpc('get_user_by_id', { p_id: user.登録番号 });
        if (data && data[0]) {
            user.連続日数 = data[0].連続日数;
            user.実施状況 = data[0].実施状況;
    ...
    user.最終実施日 = today;
    user.累計日数 = (user.累計日数 || 0) + 1;
    await updateHome();
    if (!isSpecialUser(user)) await renderCalendar(currentYear, currentMonth);
    ...
    alert(isSpecialUser(user) && !achieved ? '記録しました（3つ全部チェックすると達成になります）' : '記録しました');
}
...
function addScheduleBlockRow(name = '', startMin = 540, endMin = 600) {
    ...
    const startSel = row.querySelector('.schedule-start');
    const endSel = row.querySelector('.schedule-end');
    startSel.addEventListener('change', () => {
        const rows = document.querySelectorAll('.schedule-block-row');
        const idx = Array.from(rows).indexOf(row);
        if (idx > 0) rows[idx - 1].querySelector('.schedule-end').value = startSel.value;
        updateSchedulePreview();
    });
    endSel.addEventListener('change', () => {
        const rows = document.querySelectorAll('.schedule-block-row');
        const idx = Array.from(rows).indexOf(row);
        if (idx < rows.length - 1) rows[idx + 1].querySelector('.schedule-start').value = endSel.value;
        updateSchedulePreview();
    });
...
function saveScheduleTemplate() {
    const blocks = getScheduleBlocks().filter(b => b.name && b.start && b.end);
    if (blocks.length === 0) {
        alert('保存するブロックがありません');
        return;
    }
    localStorage.setItem('hibitan_schedule_template', JSON.stringify(blocks));
    alert('テンプレを保存しました');
}

function loadScheduleTemplate() {
    const raw = localStorage.getItem('hibitan_schedule_template');
    if (!raw) {
        alert('保存されたテンプレがありません');
        return;
    }
    const blocks = JSON.parse(raw);
    const list = document.getElementById('scheduleBlockList');
    list.innerHTML = '';
    blocks.forEach(b => addScheduleBlockRow(b.name, timeToMinutes(b.start), timeToMinutes(b.end)));
    updateSchedulePreview();
}
...
function openEditProfile() {
    ...
    const goalNamesContainer = document.getElementById('editGoalNamesContainer');
    if (isSpecialUser(user)) {
        goalNamesContainer.style.display = 'block';
        document.getElementById('editGoal2').value = user.goal2_name || '';
        document.getElementById('editGoal3').value = user.goal3_name || '';
    } else {
        goalNamesContainer.style.display = 'none';
    }
...
async function updateProfile() {
    ...
    if (isSpecialUser(user)) {
        const goal2 = document.getElementById('editGoal2').value.trim();
        const goal3 = document.getElementById('editGoal3').value.trim();

        const { error: goalError } = await supabase.rpc('update_goal_names', {
            p_id:    user.登録番号,
            p_pass:  user.パスワード,
            p_goal2: goal2,
            p_goal3: goal3
        ...
        if (goalError) {
            console.error('目標名の保存に失敗:', goalError);
            alert('目標2・3の保存に失敗しました: ' + goalError.message);
        } else {
            user.goal2_name = goal2;
            user.goal3_name = goal3;
        }
    }
...
async function showNakama(aikotoba) {
    ...
    const goalsHistoryPromises = res.data.map(u =>
        supabase
                .from('daily_records')
                .select('achieved, goals_data')
                .eq('user_id', u.登録番号)
    );
    const [countsResults, photoResults, scheduleResults, goalsHistoryResults] = await Promise.all([
        Promise.all(countsPromises),
        Promise.all(photoPromises),
        Promise.all(schedulePromises),
        Promise.all(goalsHistoryPromises)
    ]);

    // 実施日数で降順ソート
    const combined = res.data.map((u, i) => ({
        u,
        countResult:  countsResults[i],
        photoUrl:     photoResults[i]?.data?.photo_url || null,
        scheduleData: scheduleResults[i]?.data?.schedule_data || null,
        goalCounts:   tallyGoalDayCounts(goalsHistoryResults[i]?.data),
        totalDays:    u.連続日数 || 0
    })).sort((a, b) => {
        if (user && a.u.登録番号 === user.登録番号) return -1;
        if (user && b.u.登録番号 === user.登録番号) return 1;
        return 0;
    });

    for (const { u, countResult, photoUrl, scheduleData, goalCounts, totalDays } of combined) {

        const isTodayDone = u.最終実施日 === today;
        const status = isTodayDone ? '✅' : 'ー';
        const note = isTodayDone ? (u.一言日記 || '（未入力）') : 'ー';

        const stage = getCardStage(totalDays);
        const badge = CARD_BADGES[stage];

        const div = document.createElement('div');
        div.className = 'nakama-card' + (stage >= 1 ? ` nakama-card--stage-${stage}` : '');

        div.innerHTML = `
      ${badge ? `<div class="nakama-stage-badge">${badge}</div>` : ''}
      <p class="nakama-name">👤 ${u.名前}</p>
      <p class="nakama-goal">🎯 目標：${u.目標}${goalCounts[0] ? `（${goalCounts[0]}日）` : ''}</p>
      ${u.goal2_name ? `<p class="nakama-goal">🎯 目標2：${u.goal2_name}（${goalCounts[1]}日）</p>` : ''}
      ${u.goal3_name ? `<p class="nakama-goal">🎯 目標3：${u.goal3_name}（${goalCounts[2]}日）</p>` : ''}
      <p class="nakama-msg">💬 意気込み：${u.意気込み}</p>
      <p class="nakama-status">🌱 実施状況：${status}</p>
      ${isSpecialUser(u) ? '' : `<p class="nakama-streak">🔥 実施日数：${u.連続日数 || 0}日</p>`}
...
// 17. カードステージ
// =============================================
function getCardStage(totalDays) {
    if (totalDays >= 200) return 6;
    if (totalDays >= 150) return 5;
    if (totalDays >= 100) return 4;
    if (totalDays >= 50)  return 3;
    if (totalDays >= 30)  return 2;
    if (totalDays >= 10)  return 1;
    return 0;
}

const CARD_BADGES = ['研修中📋', 'アシスタント🌱', '平社員💼', 'チームリーダー🏢', '課長🏅', '部長🏆', '事業部長👑'];
        ...
        const readyReg = await Promise.race([
            new Promise(resolve => {
                ...
                sw.addEventListener('statechange', function () {
                    if (this.state === 'activated') resolve(registration);
                });
            }),
            new Promise((_, reject) => setTimeout(() => reject(new Error('SW の起動がタイムアウトしました')), 10000))
        ]);
...
async function disablePush() {
    try {
        ...
        const { error } = await supabase.rpc('delete_push_subscription', { p_user_id: user.登録番号 });
        if (error) throw new Error(error.message);

        alert('通知をOFFにしました');
    } catch (e) {
        alert('通知OFFに失敗しました: ' + e.message);
    }
}

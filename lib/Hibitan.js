...
function bindButtons() {
    ...
    if (addBlockBtn) addBlockBtn.addEventListener('click', () => addScheduleBlockRow());
...
async function initScheduleScreen() {
    ...
    if (data?.schedule_data?.length > 0) {
        data.schedule_data.forEach(b => {
            ...
            addScheduleBlockRow(b.name, startH, endH);
        });
    } else {
        addScheduleBlockRow();
    }
...
function makeHourOptions(from, to, selected) {
    return Array.from({length: to - from + 1}, (_, i) => {
        const h = from + i;
        return `<option value="${h}" ${h === selected ? 'selected' : ''}>${h}時</option>`;
    }).join('');
}

function addScheduleBlockRow(name = '', startH = 9, endH = 10) {
    const list = document.getElementById('scheduleBlockList');
    const row = document.createElement('div');
    row.className = 'schedule-block-row';
    row.innerHTML = `
        <input type="text" class="schedule-name" placeholder="活動名" maxlength="10" value="${name}">
        <select class="schedule-start">${makeHourOptions(0, 23, startH)}</select>
        <span class="schedule-wave">〜</span>
        <select class="schedule-end">${makeHourOptions(1, 24, endH)}</select>
...
function getScheduleBlocks() {
    return Array.from(document.querySelectorAll('.schedule-block-row')).map(row => ({
        ...
        start: String(parseInt(row.querySelector('.schedule-start').value)).padStart(2, '0') + ':00',
        end:   String(parseInt(row.querySelector('.schedule-end').value)).padStart(2, '0') + ':00'
    }));
}
...
function updateSchedulePreview() {
    ...
    rows.forEach((row, i) => {
        const b = blocks[i];
        const startH = parseInt(row.querySelector('.schedule-start').value);
        const endH   = parseInt(row.querySelector('.schedule-end').value);
        const isReversed = endH <= startH;

        const isOverlap = blocks.some((other, j) => {
            if (j === i) return false;
            const os = parseInt(other.start);
            const oe = parseInt(other.end);

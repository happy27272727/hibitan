...
async function loadGoalDayCounts() {
    ...
    data.forEach(row => {
        if (row.goals_data && row.goals_data.length === 3) {
            for (let i = 0; i < 3; i++) {
                if (row.goals_data[i] === true) goalDayCounts[i]++;
            }
...
async function showNakama(aikotoba) {
    ...
    for (const { u, countResult, photoUrl, scheduleData, totalDays } of combined) {
      ...
      ${u.goal2_name ? `<p class="nakama-goal">🎯 目標2：${u.goal2_name}</p>` : ''}
      ${u.goal3_name ? `<p class="nakama-goal">🎯 目標3：${u.goal3_name}</p>` : ''}

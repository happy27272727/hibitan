...
async function enablePush() {
    ...
    await supabase
            ...
            .upsert(
                    ...
                    {onConflict: ['user_id']} // ← user_id が既にあれば上書き
            );
...
var seityouBtn = document.getElementById('seityouBtn');
if (seityouBtn) {
    seityouBtn.addEventListener('click', showSeityouKiroku);
}



// 通知設定画面を表示
var pushSettingsBtn = document.getElementById('pushSettingsBtn'); // 通知設定を開くボタン
if (pushSettingsBtn) {
    pushSettingsBtn.addEventListener('click', () => {
        showScreen('pushSettingsScreen'); // 画面切替関数を使う
    });
}

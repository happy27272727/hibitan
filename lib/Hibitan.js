...
async function showNakama(aikotoba) {
    ...
    for (const { u, countResult, photoUrl, scheduleData, goalCounts, totalDays } of combined) {
      ...
      <p class="nakama-goal">🎯 目標：${u.目標}${goalCounts[0] ? `（${goalCounts[0]}日達成）` : ''}</p>
      ${u.goal2_name ? `<p class="nakama-goal">🎯 目標2：${u.goal2_name}（${goalCounts[1]}日達成）</p>` : ''}
      ${u.goal3_name ? `<p class="nakama-goal">🎯 目標3：${u.goal3_name}（${goalCounts[2]}日達成）</p>` : ''}

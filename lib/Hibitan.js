...
// JST基準の今日の日付を取得（YYYY-MM-DDaaa
function getTodayJST() {
    const now = new Date(); // ブラウザのローカル時間（日本）
    const y = now.getFullYear();
    const m = String(now.getMonth() + 1).padStart(2, '0');
    const d = String(now.getDate()).padStart(2, '0');
    return `${y}-${m}-${d}`; // JST基準の日付
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (roguinBtn)
        roguinBtn.addEventListener('click', loginButtonClick);

    const tasseiBtn = document.getElementById('tasseiButton');
    if (tasseiBtn)
        tasseiBtn.addEventListener('click', tasseiButtonClick);
...
async function loginButtonClick() {
    ...
    if (mokuhyouHyouziEl)
        mokuhyouHyouziEl.textContent = `目標: ${user.目標}`;
    if (ikigomiHyouziEl)
        ikigomiHyouziEl.textContent = `意気込み: ${user.意気込み}`;
    if (renzokuHyouziEl)
        renzokuHyouziEl.textContent = `連続日数: ${user.連続日数 || 0}日`;
...
async function tasseiButtonClick() {
    ...
    const lastDate = userData.最終実施日
    ? userData.最終実施日.split('T')[0]
    : null;
...
if (lastDate === yesterday) {
    ...
    if (ikigomiHyouziEl)
        ikigomiHyouziEl.textContent = `意気込み: ${user.意気込み}`;
    if (renzokuHyouziEl)
        renzokuHyouziEl.textContent = `連続日数: ${user.連続日数}日`;
...
//    const tuutiZikan = document.getElementById("tuutiZikan").value;
    ...
    if (!nameInput)
        missingFields.push("ニックネーム");
    if (!password)
        missingFields.push("パスワード");
    if (!mokuhyouInput)
        missingFields.push("目標");
    if (!aikotoba)
        missingFields.push("合言葉");
    if (!ikigomi)
        missingFields.push("意気込み");
...
async function showNakama(aikotoba) {
    ...
    data.forEach(user => {
        const lastDate = user.最終実施日
                ? user.最終実施日.split('T')[0]
                : null;
    ...
    <p>一言日記: ${isToday && user.一言日記 ? user.一言日記 : '未記入'}</p>
`;
